...
tCandidato* CriaCandidato(){
    ...
    cand->nome = (char*) calloc(2, sizeof(char));
    cand->partido = (char*) calloc(2, sizeof(char));
...
void LeCandidato(tCandidato *candidato){
    ...
    while((scanf("%c", &candidato->nome[i])) == 1 && candidato->nome[i] != ','){
        i++;
        candidato->nome = (char*) realloc(candidato->nome, (i+2)*sizeof(char));
    }

    candidato->nome[i] = '\0';
    scanf(" ");

    while((scanf("%c", &candidato->partido[j])) == 1 && candidato->partido[j] != ','){
        j++;
        candidato->partido = (char*) realloc(candidato->partido, (j+2)*sizeof(char));
    }

    candidato->partido[j] = '\0';
...
float CalculaPercentualVotos(tCandidato* candidato, int totalVotos){
    ...
    percent = (candidato->votos*100) / total;
    return percent;
}
