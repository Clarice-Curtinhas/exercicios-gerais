...
int SalvaAluno(tAluno *aluno, FILE *file){
    ...
    tam = sizeof(aluno->nome)+1;
    fwrite(&tam, sizeof(int), 1, file);
    numBytes += fwrite(aluno->nome, sizeof(char), tam, file);

    tam = sizeof(aluno->data)+1;
    fwrite(&tam, sizeof(int), 1, file);
    numBytes += fwrite(aluno->data, sizeof(char), tam, file);

    tam = sizeof(aluno->curso)+1;
    fwrite(&tam, sizeof(int), 1, file);
    numBytes += fwrite(aluno->curso, sizeof(char), tam, file);

    fwrite(&aluno->percent, sizeof(float), 1, file);
...
int CarregaAluno(tAluno *aluno, FILE *file){
    ...
    numBytes += fread(aluno->nome, sizeof(char), tam, file);

    fread(&tam, sizeof(int), 1, file);
    numBytes += fread(aluno->data, sizeof(char), tam, file);

    fread(&tam, sizeof(int), 1, file);
    numBytes += fread(aluno->curso, sizeof(char), tam, file);
